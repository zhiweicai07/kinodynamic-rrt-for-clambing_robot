import yaml
import math
import shapely.geometry as geom
from shapely import affinity
import itertools
from matplotlib import pyplot as plt
from descartes import PolygonPatch


def plot_environment(env, bounds=None, figsize=None):   # 画障碍物图
    if bounds is None and env.bounds:
        minx, miny, maxx, maxy = env.bounds
    elif bounds:
        minx, miny, maxx, maxy = bounds
    else:
        minx, miny, maxx, maxy = (-10, -5, 10, 5)

    # 测试bounds是什么
    print("minx=", minx)
    print("miny=", miny)
    print("maxx=", maxx)
    print("maxy=", maxy)

    max_width, max_height = 12, 5.5
    if figsize is None:
        width, height = max_width, (maxy-miny)*max_width/(maxx-minx)
        if height > 5:
            width, height = (maxx-minx)*max_height/(maxy-miny), max_height
        figsize = (width, height)   # figsize=(7.0, 5.5)
    print("figsize=", figsize)
    f = plt.figure(figsize=figsize)
    ax = f.add_subplot(111)
    for i, obs in enumerate(env.obstacles):
        # print("i", i, obs)
        patch = PolygonPatch(obs, fc='blue', ec='blue', alpha=0.5, zorder=20)
        ax.add_patch(patch)

    plt.xlim([minx, maxx])
    plt.ylim([miny, maxy])
    ax.set_aspect('equal', adjustable='box')
    return ax

# def plot_environment_on_axes(ax, env, bounds=None, figsize=None):
#     if bounds is None and env.bounds:
#         minx, miny, maxx, maxy = env.bounds
#     elif bounds:
#         minx, miny, maxx, maxy = bounds
#     else:
#         minx, miny, maxx, maxy = (-10,-5,10,5)
#
#     max_width, max_height = 12, 5.5
#     if figsize is None:
#         width, height = max_width, (maxy-miny)*max_width/(maxx-minx)
#         if height > 5:
#             width, height = (maxx-minx)*max_height/(maxy-miny), max_height
#         figsize = (width, height)
#     #print(figsize)
#     for i, obs in enumerate(env.obstacles):
#         patch = PolygonPatch(obs, fc='blue', ec='blue', alpha=0.5, zorder=20)
#         ax.add_patch(patch)
#
#     plt.xlim([minx, maxx])
#     plt.ylim([miny, maxy])
#     #ax.set_aspect('equal', adjustable='box')

# def plot_line(ax, line):
#     x, y = line.xy
#     ax.plot(x, y, color='gray', linewidth=1, solid_capstyle='round', zorder=1)


def plot_poly(ax, poly, color, alpha=1.0, zorder=1):
    patch = PolygonPatch(poly, fc=color, ec="black", alpha=alpha, zorder=zorder)
    ax.add_patch(patch)

class Environment:

    def __init__(self, yaml_file=None, bounds=None):    # 构造函数初始化类的实例
        self.yaml_file = yaml_file
        self.environment_loaded = False     # 环境未加载
        self.obstacles = []
        self.obstacles_map = {}
        self.bounds = bounds
        if not yaml_file is None:
            if self.load_from_yaml_file(yaml_file):
                if bounds is None:
                    self.calculate_scene_dimensions()   # 计算环境的边界
                self.environment_loaded = True      # 环境加载成功

    #@property
    #def bounds(self):
    #    return self.bounds

    # def add_obstacles(self, obstacles):
    #     self.obstacles = self.obstacles + obstacles
    #     self.calculate_scene_dimensions()

    def calculate_scene_dimensions(self):
        """
        Compute scene bounds from obstacles
        计算场景边界
        """
        points = []
        for elem in self.obstacles:
            points = points + list(elem.boundary.coords)

        mp = geom.MultiPoint(points)        # mp对象包含了障碍物的边界坐标点
        self.bounds = mp.bounds

    def load_from_yaml_file(self, yaml_file):   # 加载yaml文件
        f = open(yaml_file)     # 打开yaml文件并创建f对象
        self.data = yaml.safe_load(f)   # 从f对象安全加载yaml数据，并储存到类实例data属性中
        f.close()
        return self.parse_yaml_data(self.data)  # 最后返回True或者False

    def parse_yaml_data(self, data):    # 解析yaml加载的数据
        if 'environment' in data:   # yaml数据是键值对
            env = data['environment']   # 对应的值赋给env
            self.parse_yaml_obstacles(env['obstacles'])     # 解析yaml障碍物数据
            # self.parse_yaml_features(env['features'])
            return True     # 解析成功返回True
        else:
            return False

    def parse_yaml_obstacles(self, obstacles):
        self.obstacles = []
        self.obstacles_map = {}
        if obstacles is not None:
            for name, description in obstacles.items():     # 在每次循环迭代时，name 变量存储键，description 变量存储值
                # Double underscore not allowed in region names     名称中不允许使用双下划线__
                if name.find("__") != -1:
                    raise Exception("Names cannot contain double underscores.")
                if description['shape'] == 'rectangle':
                    parsed = self.parse_rectangle(name, description)
                elif description['shape'] == 'polygon':
                    parsed = self.parse_polygon(name, description)
                else:
                    raise Exception("not a rectangle")
                if not parsed.is_valid:
                    raise Exception("%s is not valid!"%name)
                self.obstacles.append(parsed)
                self.obstacles_map[name] = parsed
            self.expanded_obstacles = [obs.buffer(0.75/2, resolution=2) for obs in self.obstacles]

    def parse_rectangle(self, name, description):       # 矩形障碍物解析
        # yaml文件中设置矩形障碍物的各种参数
        center = description['center']
        center = geom.Point((center[0], center[1]))
        length = description['length']
        width = description['width']
        # convert rotation to radians   角度转化成弧度
        rotation = description['rotation']  # math.pi/180
        # figure out the four corners   算出四个顶点
        corners = [(center.x - length/2., center.y - width/2.),
                   (center.x + length/2., center.y - width/2.),
                   (center.x + length/2., center.y + width/2.),
                   (center.x - length/2., center.y + width/2.)]
        # print corners    将四个顶点组成多边形
        polygon = geom.Polygon(corners)
        out = affinity.rotate(polygon, rotation, origin=center)
        out.name = name
        out.cc_length = length
        out.cc_width = width
        out.cc_rotation = rotation
        return out

    def parse_polygon(self, name, description):     # 排列组合
        # 多边形障碍物解析
        # name表示多边形的名称，description是包含多边形描述信息的字典
        _points = description['corners']
        for points in itertools.permutations(_points):  # 对顶点坐标进行排列组合，生成所有可能的顶点排列
            polygon = geom.Polygon(points)
            # polygon.name = name
            if polygon.is_valid:
                return polygon

    # def save_to_yaml(self, yaml_file):
    #     yaml_dict = {}
    #     obstacles = {}
    #     for i, ob in enumerate(self.obstacles):
    #         ob_dict = {}
    #         ob_dict['shape'] = 'polygon'
    #         ob_dict['corners'] = [list(t) for t in list(ob.boundary.coords)]
    #         ob_name = "obstacle%.4d"%i
    #         obstacles[ob_name] = ob_dict
    #     yaml_dict['environment'] = {'obstacles' : obstacles}
    #
    #     f = open(yaml_file, 'w')
    #     f.write(yaml.dump(yaml_dict, default_flow_style=None))
    #     f.close()